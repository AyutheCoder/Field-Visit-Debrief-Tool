...
// --- Users ---
const officer = createUser({
    name: 'Asha Mwangi',
    email: 'asha@fieldteam.org',
    role: 'field_officer',
});

const officer2 = createUser({
    name: 'Daniel Okoro',
    email: 'daniel@fieldteam.org',
    role: 'field_officer',
});

createUser({ name: 'Priya Nair', email: 'priya@fieldteam.org', role: 'manager' });
createUser({ name: 'Sam Carter', email: 'admin@fieldteam.org', role: 'admin' });
...
for (const v of visits) {
    ...
    const debrief = upsertDebrief(visit.id, {
        ...v.debrief,
        aiModel: 'seed',
        editedByHuman: false,
    });
